# C2 — Advection–diffusion: when Galerkin fails, and SUPG

LEARNING OUTCOME. You can recognize the failure mode of plain Galerkin on
...
BACKGROUND. u_t + a.grad(u) - kappa lap(u) = f. The SUPG-stabilized weak
form adds, per element,
    sum_e ( tau (a.grad w),  R(u) )_e,   R = u_t + a.grad u - kappa lap u - f
with tau from the same VMS toolbox track D uses (diffsim.physics.vms,
metric form). At p1 the lap(u_h) term in R vanishes elementwise (Q1's
...
EXPECTED RESULTS (kappa = 1e-3, |a| = 1 — cell Peclet >> 1 at level 5;
...
Run:  python tutorials/C_time/C2_advection_diffusion.py

??? example "Full script — `tutorials/C_time/C2_advection_diffusion.py` (run it!)"
