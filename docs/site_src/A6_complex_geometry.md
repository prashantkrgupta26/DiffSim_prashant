# A6 — Carving domains: a channel, a sphere, and a triangle mesh

LEARNING OUTCOME. You can build computational domains by COMPOSITION of
...
BACKGROUND. Three carves, same physics (the A1 Poisson MMS, order 2 each):
 ...
 3. TRIANGLE MESH: an icosphere (a triangulated sphere) through the
    ...
    For a real file: `verts, tris = your_stl_loader(path)` then
...
Run:  python tutorials/A_foundations/A6_complex_geometry.py

??? example "Full script — `tutorials/A_foundations/A6_complex_geometry.py` (run it!)"
