# C1 — Time: the transient heat equation with BDF1 and BDF2

LEARNING OUTCOME. You can discretize du/dt with backward-difference
...
local O(dt^2) does not spoil global order 2), and (ii) spatial error
pollutes dt-ladders unless you compare against a fine-dt reference ON THE
SAME MESH.

BACKGROUND.  u_t - lap(u) = f,  u = g on the boundary, u(0) = u0. The BDF
discretization turns each step into a Poisson-like solve:
    (b0/dt) M u^{n+1} + K u^{n+1} = F(t^{n+1}) - M (b1 u^n + b2 u^{n-1})/dt
...
EXPECTED RESULTS (level 5 mesh, T = 0.5, reference dt = T/512):
    BDF1: rates -> ~1.0        BDF2: rates -> ~2.0
    and BDF2's error at dt = T/32 beats BDF1's at dt = T/128.

Run:  python tutorials/C_time/C1_heat_bdf.py

??? example "Full script — `tutorials/C_time/C1_heat_bdf.py` (run it!)"
