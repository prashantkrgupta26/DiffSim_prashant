# B1 — Nonlinearity: the Bratu problem and Newton's method

LEARNING OUTCOME. You can take a solver you trust for LINEAR problems and
...
BACKGROUND. The Bratu (solid-fuel ignition) problem:
    -lap(u) = lam * exp(u) + f,     u = g on the boundary.
Weak residual and Jacobian at the current iterate u_h:
    R_a(u) = (grad N_a, grad u_h) - (N_a, lam e^{u_h} + f)
    J_ab   = (grad N_a, grad N_b) - (N_a, lam e^{u_h} N_b)
J is the stiffness matrix MINUS an exp-weighted mass matrix — we assemble
that weighted mass right here in ~10 lines of numpy einsum over element
...
Run:  python tutorials/B_nonlinear/B1_bratu_newton.py

??? example "Full script — `tutorials/B_nonlinear/B1_bratu_newton.py` (run it!)"
