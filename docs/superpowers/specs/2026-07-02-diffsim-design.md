...
**Constraint chains (M0.5 finding).** The one-knob rule governs faces, not corners: in a legal mixed mesh, a p-hanging node's same-level p1 owner can itself have h-hanging corner nodes via a *diagonal* coarser neighbor — a p-hanging → h-hanging → free constraint chain the §13.3 mixed fuzz battery exposed (the pure-h "owner corners are free under 2:1 balance" theorem does not extend to p-transitions). Resolution: **transitive constraint closure** at T-build time — raw rows may target hanging nodes; a fixed-point substitution pass replaces each hanging target with its own row until all targets are free (chain length ≤ 2 under one-knob + 2:1; the loop is a no-op for p1-only and p2-only meshes). Substitution is exact: the chained row is the composition of pointwise trace evaluations, so PoU (row sums = 1) and linear reproduction hold at machine precision through chains. Zero hot-path cost — T remains a plain CSR; only constraint assembly (setup/adaptivity epochs) pays the extra passes. Pinned by `test_chain_closure_deterministic` (structural chain-presence guard + PoU + reproduction) in addition to the fuzz batteries.
