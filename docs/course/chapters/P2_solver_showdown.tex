\chapter{Solver showdown: direct vs iterative vs single-sync iterative}
\label{ch:P2_solver_showdown}
\noindent\texttt{tutorials/P\_performance/P2\_solver\_showdown.py}\\[0.75em]
\begin{outcome}
You can choose a linear solver from MEASUREMENT, not
...
Three ways to solve the same SPD Poisson system:
  1. splu           — O(n\textasciicircum{}1.5) flops (2-D), unbeatable small, memory-bound
                      large, refactorize every time the matrix changes.
  2. host CG        — O(n) per iteration x O(h\textasciicircum{}-1) iterations (Jacobi),
                      but EVERY dot product ships a scalar to the host and
                      stalls the pipeline (\textasciitilde{}6 syncs / iteration).
  3. fused CG (diffsim.solvers.krylov\_dev) — same math, all reduction
                      scalars stay on the device; ONE readback per
                      check\_every iterations. m1b findings 2: measured
