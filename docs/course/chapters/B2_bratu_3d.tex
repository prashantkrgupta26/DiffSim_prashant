\chapter{Bratu in 3-D + continuation toward the fold}
\label{ch:B2_bratu_3d}
\noindent\texttt{tutorials/B\_nonlinear/B2\_bratu\_3d.py}\\[0.75em]
\begin{outcome}
You can (i) carry a nonlinear solver to 3-D unchanged,
...
Same equations as B1 with dim=3 (the physical problem now:
f = 0, g = 0, so the trivial branch bends at a critical lam*). For the
unit cube lam* \textasciitilde{} 9.9. We march lam = 1, 2, ..., warm-starting from the
