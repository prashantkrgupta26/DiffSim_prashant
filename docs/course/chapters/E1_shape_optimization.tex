\chapter{Differentiable simulation: find the hidden circle}
\label{ch:E1_shape_optimization}
\noindent\texttt{tutorials/E\_differentiable/E1\_shape\_optimization.py}\\[0.75em]
\begin{outcome}
You can close the design loop: define a quantity of
...
u(x\_i) from inside. Recover the disk.
...
every iteration. Nothing here is finite-differenced; the gradient chain is
    ...
    J -> dJ/du            (probe least squares, sparse point-evaluation)
      -> adjoint solve    A\textasciicircum{}T lam = dJ/du        (one transposed solve)
      -> -lam\textasciicircum{}T dR/dtheta (a Warp TAPE over the SBM face-residual kernels:
