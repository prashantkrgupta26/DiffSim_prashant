...
def shape_gradient(problem, u_all, lam_free, oracle, meta,
    ...
    if problem.dir is not None:
        ...
        b = dm.bins[fs.pv]
        nqf = fs.ftab.nqf
        # g_fn=None is HOMOGENEOUS Dirichlet in the forward path
        # (_dirichlet_rhs_kappa1) — mirror it here: gbar = 0, and the
        # g-chain term vanishes (d(0)/dtheta = 0). Previously this line
        # crashed with NoneType (evaluation finding 2, CONFIRMED).
        if problem.g_fn is None:
            gbar_np = np.zeros(len(fs.geo.xq), np.float64)
        else:
        ...
        y_t = xq_t + d_t
        if problem.g_fn is None:
            gbar_t = torch.zeros(len(fs.geo.xq), dtype=torch.float64)
        elif g_fn_torch is not None:
