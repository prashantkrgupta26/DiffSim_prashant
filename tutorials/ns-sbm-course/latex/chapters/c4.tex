...
obstacle-outward normal is $\hat n=-\code{geo.n}$ and the integrand is
$+p\,\code{geo.n}-\nu(\grad u)^\top\code{geo.n}$. At $d=0$ this reduces to
...
\code{--zero-shift} to set $\code{geo.d}=0$, $\code{geo.corr}=1$ while
