\chapter{The MMS discipline: manufactured solutions and convergence orders}
\label{ch:A1_mms_convergence}
\noindent\texttt{tutorials/A\_foundations/A1\_mms\_convergence.py}\\[0.75em]
\begin{outcome}
You can verify a solver the way this group verifies
...
Solve -lap(u) = f on the unit square with u = g on the boundary
...
u*(x,y) = sin(pi x) sin(pi y), then f = 2 pi\textasciicircum{}2 u* and g = u* = 0 on the
boundary. The discrete error should shrink as O(h\textasciicircum{}\{p+1\}) in L2 — order 2
