\chapter{The cost model: per-stage scaling, measured exponents, memory}
\label{ch:P1_cost_model_and_scaling}
\noindent\texttt{tutorials/P\_performance/P1\_cost\_model\_and\_scaling.py}\\[0.75em]
\begin{outcome}
You can do for this 2-D/GPU stack exactly what
FEM\_computational\_cost.pdf does for the 1-D CPU solver: predict each
...
— the analytical table you are testing (n = number of DOFs):
    ...
    CSR triplets -> matrix (host)  O(n)               O(n) (\textasciitilde{}9n nnz p1 2-D)
    splu factorization (2-D)       O(n\textasciicircum{}1.5)           O(n log n) fill
    triangular solves              O(n log n)         —
GPU-specific effects the 1-D document could not show you:
  (i) LAUNCH FLOOR: below \textasciitilde{}1e5 elements a kernel launch costs more than
...
(iii) HOST<->DEVICE SYNC: every .numpy() readback stalls the GPU pipeline
      (m1a findings 3 measured \textasciitilde{}10 ms under load on WSL2).
