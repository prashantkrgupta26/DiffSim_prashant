...
def test_g4_learn_free_energy(device):
    """G4 (application c): learn a bulk free energy f(c) from a TRAJECTORY of
    morphology snapshots.  Truth f'(c)=c^3-c on a monomial basis; recover the
    coefficients by L-BFGS using the G1 adjoint.  (i) the trajectory-loss
    gradient matches FD; (ii) clean recovery to ~1e-6; (iii) noisy recovery
    improves markedly with more snapshots."""
    from scipy.optimize import minimize
    from diffsim.adjoint.phasefield import PolyBasisEnergy
    dm, mesh = _dm(3, device)
    coords = mesh.node_coords
    nn = dm.n_nodes
    powers, truth = [3, 1], np.array([1.0, -1.0])
    M0, kap, dt, nst = 1.0, 0.005, 0.005, 10
    c0 = 0.4 * np.cos(np.pi * coords[:, 0]) * np.cos(np.pi * coords[:, 1])

    def gen(a):
        f = CHForward(dm, PolyBasisEnergy(powers, a), M=M0, kappa=kap, dt=dt,
                      order=2)
        f.set_initial(c0); f.run(nst)
        return f
    data = [gen(truth).steps[n]["c"].copy() for n in range(nst)]
    ...
    def lg(a, snaps, dat):
        ...
        for n in snaps:
            r = f.steps[n]["c"] - dat[n]
            L += 0.5 * float(r @ r); dJ[n][0::2] = r
        g = CHAdjoint(f).gradient(dJ, ["a0", "a1"])
        return L, np.array([g["a0"], g["a1"]])

    # (i) gradient vs FD
    at = np.array([0.7, -1.3])
    snaps = list(range(nst))
    _, g_adj = lg(at, snaps, data)
    for i in range(2):
        e = 1e-6
        hi = at.copy(); hi[i] += e
        lo = at.copy(); lo[i] -= e
        gfd = (lg(hi, snaps, data)[0] - lg(lo, snaps, data)[0]) / (2 * e)
        r = abs(g_adj[i] - gfd) / max(abs(gfd), 1e-14)
        print(f"G4 grad da{i}: adj={g_adj[i]:+.6e} fd={gfd:+.6e} rel={r:.2e}")
        assert r < 1e-6, (i, g_adj[i], gfd)

    # (ii) clean recovery
    a0 = np.array([0.5, -0.5])
    res = minimize(lambda a: lg(a, snaps, data), a0, jac=True,
                   method="L-BFGS-B", options=dict(maxiter=200, ftol=1e-16))
    err = np.abs(res.x - truth).max()
    print(f"G4 clean recovery: a={res.x} |a-truth|={err:.2e} loss={res.fun:.2e}")
    assert err < 1e-6, (res.x, err)

    # (iii) noisy recovery improves with more snapshots
    rng = np.random.default_rng(0)
    noisy = [data[n] + 0.01 * rng.standard_normal(nn) for n in range(nst)]
    errs = {}
    for K in (1, 4):
        s = list(np.linspace(0, nst - 1, K).astype(int))
        r = minimize(lambda a: lg(a, s, noisy), a0, jac=True,
                     method="L-BFGS-B", options=dict(maxiter=200, ftol=1e-15))
        errs[K] = np.abs(r.x - truth).max()
    print(f"G4 noisy recovery: K=1 err={errs[1]:.4f} K=4 err={errs[4]:.4f}")
    assert errs[4] < 0.3 * errs[1], errs   # more data -> better recovery


def test_g5_design_process_route(device):
    """G5 (application d): PDE-constrained design of a processing route —
    optimise the temperature schedule T(t) to hit a TARGET crystalline
    fraction (mean psi) using the G3 schedule gradient + L-BFGS.  Target is
    generated by a ground-truth ramp (reachable by construction); achieved
    must converge to it."""
    from scipy.optimize import minimize
    from diffsim.adjoint.crystallization import CACHForward, CACHAdjoint
    dm, mesh = _dm(3, device)
    ...
    NF = 3
    cc = np.cos(np.pi * coords[:, 0]) * np.cos(np.pi * coords[:, 1])
    phi0, psi0 = 0.5 + 0.1 * cc, 0.2 + 0.05 * cc
    base = dict(M=1.0, kappa=0.02, eps2=0.02, L=1.0, dsig=1.0, dh=1.0, Tm=2.0)
    dt, NS = 0.02, 6

    def march(Ts):
        f = CACHForward(dm, FHEnergy(1.0, 2.5), M=base["M"], kappa=base["kappa"],
                        eps2=base["eps2"], L=base["L"], dsig=base["dsig"],
                        dh=base["dh"], Tm=base["Tm"], T=Ts[0], dt=dt, order=2)
        f.set_initial(phi0, psi0)
        f.set_schedule(list(Ts), bT=0.0)
        f.run(NS)
        return f
    frac = lambda f: float(np.mean(f.steps[-1]["x"][2::NF]))

    T_gt = np.linspace(0.5, 0.3, NS)     # ground-truth cold ramp
    target = frac(march(T_gt))

    def og(Ts):
        f = march(Ts)
        m = frac(f)
        J = 0.5 * (m - target) ** 2
        dJ = [np.zeros(NF * nn) for _ in range(NS)]
        dJ[-1][2::NF] = (m - target) / nn
        return J, CACHAdjoint(f).temperature_gradient(dJ)

    # gradient sanity vs FD
    T0 = np.full(NS, 1.0)
    J0, g = og(T0)
    for i in range(NS):
        e = 1e-6
        hi = T0.copy(); hi[i] += e
        lo = T0.copy(); lo[i] -= e
        gfd = (og(hi)[0] - og(lo)[0]) / (2 * e)
        assert abs(g[i] - gfd) < 1e-6 * max(abs(gfd), 1e-8), (i, g[i], gfd)

    res = minimize(og, T0, jac=True, method="L-BFGS-B",
                   bounds=[(0.2, 2.0)] * NS,
                   options=dict(maxiter=200, ftol=1e-20, gtol=1e-15))
    achieved = frac(march(res.x))
    print(f"G5 target={target:.5f} achieved={achieved:.5f} "
          f"|diff|={abs(achieved-target):.2e} J {J0:.2e}->{res.fun:.2e}")
    assert abs(achieved - target) < 1e-6, (achieved, target)
    assert res.fun < 1e-10 * max(J0, 1e-30) or res.fun < 1e-18
